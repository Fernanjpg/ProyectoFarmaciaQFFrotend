...
export default function ProductTypeTable({ productTypes, onAddUserClick }) {
  // Helpers to pick colors based on role/status
  const getRoleColors = (productType) => {
    switch(productType) {
    ...
    <div className="w-full">
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 mb-2 tracking-widest uppercase">
            <span>Portal</span>
            <span>{'>'}</span>
            <span className="text-[#1a4d3a]">Gestión de Tipos de Productos</span>
          </div>
          <h1 className="text-4xl font-bold text-slate-800 tracking-tight">Gestión de Tipos de Productos</h1>
          <p className="text-slate-500 mt-2 font-medium">Control centralizado de tipos de productos para la cadena logística.</p>
        </div>
      ...
      <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
          ...
          <table className="w-full text-left text-sm whitespace-nowrap min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-50">
              {productTypes.map(productType => (
                <tr key={productType.id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="py-5 px-8">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold text-[13px] bg-slate-100 text-slate-600 shadow-inner border border-white">
                        {getInitials(productType.nombre, productType.apellido)}
                      </div>
                      <div>
                        <p className="font-bold text-slate-800 text-[15px]">{productType.nombre} {productType.apellido}</p>
                        <p className="text-[11px] font-semibold text-slate-400 mt-0.5">ID-{productType.sucursal ? productType.sucursal.substring(0, 2).toUpperCase() : 'QF'}-{productType.id.toString().padStart(4, '0')}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-5 px-8">
                    <span className={`px-3 py-1.5 rounded-lg text-xs font-bold inline-block border border-black/5 ${getRoleColors(productType)} uppercase tracking-wider`}>
                      {productType.rol}
                    </span>
                  </td>
                  <td className="py-5 px-8 text-slate-500 font-medium">{productType.correo}</td>
                  <td className="py-5 px-8">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${getStatusColor(productType.estado).split(' ')[0]}`}></div>
                      <span className={`text-[11px] tracking-wider font-bold uppercase ${getStatusColor(productType.estado).split(' ')[1]}`}>
                        {productType.estado || 'INACTIVO'}
                      </span>
                    </div>
                  </td>
              ...
              {productTypes.length === 0 && (
                <tr>
                  <td colSpan="5" className="py-8 text-center text-slate-500 font-medium">No se encontraron tipos de productos.</td>
                </tr>
              )}
            </tbody>
          </table>
        ...
        <div className="bg-slate-50/50 border-t border-slate-100 py-4 px-8 flex justify-between items-center text-sm font-medium text-slate-500">
          <p>Mostrando {productTypes.length} tipos de productos operativos</p>
          <div className="flex items-center gap-2">
