...
export default function RoleTable({ roles, onAddUserClick }) {
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-50">
                  ...
                  <td className="py-5 px-8">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-full flex items-center justify-center font-bold text-[13px] bg-slate-100 text-slate-600 shadow-inner border border-white">
                        {getInitials(role.nombre, role.apellido)}
                      </div>
                      <div>
                        <p className="font-bold text-slate-800 text-[15px]">{role.nombre} {role.apellido}</p>
                        <p className="text-[11px] font-semibold text-slate-400 mt-0.5">ID-{role.sucursal ? role.sucursal.substring(0, 2).toUpperCase() : 'QF'}-{role.id.toString().padStart(4, '0')}</p>
                      </div>
                    </div>
                  </td>
                  <td className="py-5 px-8">
                    <span className={`px-3 py-1.5 rounded-lg text-xs font-bold inline-block border border-black/5 ${getRoleColors(role.rol)} uppercase tracking-wider`}>
                      {role.rol}
                    </span>
                  </td>
                  <td className="py-5 px-8 text-slate-500 font-medium">{role.correo}</td>
                  <td className="py-5 px-8">
              ...
              {roles.length === 0 && (
                <tr>
                  <td colSpan="5" className="py-8 text-center text-slate-500 font-medium">No se encontraron roles.</td>
                </tr>
              )}
            </tbody>
          </table>
