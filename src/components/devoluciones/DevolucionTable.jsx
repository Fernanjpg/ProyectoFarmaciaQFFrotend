import { Search, Plus, Trash2, Edit3, RotateCcw } from 'lucide-react';

export default function DevolucionTable({ devoluciones, onAddDevolucionClick, onDeleteDevolucion }) {
    ...
    <div className="bg-white rounded-[40px] border border-slate-100 overflow-hidden shadow-[0_20px_50px_rgba(0,0,0,0.02)] animate-in fade-in slide-in-from-bottom-8 duration-1000">
      <div className="p-8 lg:p-10 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/30">
        <div className="flex items-center gap-5">
          ...
          <div>
            <h2 className="text-2xl font-black text-slate-800 tracking-tight">Devoluciones</h2>
            <p className="text-slate-400 text-sm font-medium mt-0.5">Gestión de devoluciones de productos</p>
          </div>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50">
              ...
              <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Fecha Devolución</th>
            ...
            {devoluciones.length === 0 ? (
              <tr>
                <td colSpan="5" className="px-10 py-16 text-center">
                  <p className="text-slate-400 font-bold text-sm">No hay registros de devoluciones</p>
                </td>
              </tr>
            ) : (
              devoluciones.map((dev) => (
                <tr key={dev.idDevolucion} className="group hover:bg-slate-50/50 transition-all duration-200">
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-slate-100 rounded-xl flex items-center justify-center font-black text-[#1a4d3a] text-xs">
                        {dev.nombreProducto?.substring(0, 2).toUpperCase() || 'N/A'}
                      </div>
                      <p className="text-sm font-black text-slate-700 group-hover:text-[#499D81] transition-colors">{dev.nombreProducto || 'SIN PRODUCTO'}</p>
                    </div>
                  </td>
                  <td className="px-10 py-6 text-sm text-slate-500 font-bold">{dev.cantidad || 0}</td>
                  <td className="px-10 py-6 text-sm text-slate-500 font-bold truncate max-w-xs">{dev.motivo || 'N/A'}</td>
                  <td className="px-10 py-6 text-sm text-slate-500 font-bold">{formatDate(dev.fechaDevolucion)}</td>
                  <td className="px-10 py-6">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <button
                        onClick={() => onDeleteDevolucion(dev.idDevolucion)}
