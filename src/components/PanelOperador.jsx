...
import {
  ...
  Inbox, Send, Truck, ArrowDown, ArrowUp
} from 'lucide-react';

const PanelOperador = () => {
  const navigate = useNavigate();
  const [activeSidebar, setActiveSidebar] = useState('Entradas');
  ...
  const mockData = {
    Entrada: [
      { id: 'IN-001', fecha: '2024-04-24', actividad: 'Recepción de medicamentos A1', estado: 'Completado' },
      { id: 'IN-002', fecha: '2024-04-24', actividad: 'Ingreso de insumos médicos', estado: 'En proceso' },
    ],
    Salida: [
      { id: 'OUT-001', fecha: '2024-04-24', actividad: 'Despacho a Farmacia Central', estado: 'Completado' },
      { id: 'OUT-002', fecha: '2024-04-24', actividad: 'Envío de vacunas', estado: 'Pendiente' },
    ],
    Diarios: [
      { id: 'OP-001', fecha: '2024-04-24', actividad: 'Control de temperatura sector B', estado: 'Completado' },
      { id: 'OP-002', fecha: '2024-04-24', actividad: 'Inventario rápido de urgencias', estado: 'En proceso' },
    ],
    Dañados: [
      { id: 'DMG-001', fecha: '2024-04-24', actividad: 'Reporte de frascos rotos lote 4B', estado: 'Finalizado' },
      { id: 'DMG-002', fecha: '2024-04-24', actividad: 'Cajas húmedas sector C', estado: 'Pendiente' },
    ]
  };
      ...
      <aside className="w-72 bg-[#f4fbf8] text-slate-700 flex flex-col h-full border-r border-[#d1ebe0] z-20 shrink-0">
        ...
        <nav className="flex-1 px-4 mt-8">
          <SidebarItem 
            icon={Inbox} 
            label="Entradas" 
            active={activeSidebar === 'Entradas'} 
            onClick={() => setActiveSidebar('Entradas')} 
          />
          <SidebarItem 
            icon={Send} 
            label="Salidas" 
            active={activeSidebar === 'Salidas'} 
            onClick={() => setActiveSidebar('Salidas')} 
          />
          <SidebarItem 
            icon={ClipboardList} 
            label="Reportes" 
          ...
          <header className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10 bg-slate-50/50 p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden group">
            ...
            <div className="relative z-10">
              <h1 className="text-3xl font-black text-slate-800 leading-none">
                Bienvenido, <span className="text-[#499D81]">{user?.username || 'Operador'}</span>
              </h1>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 bg-green-50 rounded-2xl flex items-center justify-center border border-green-100">
                  {activeSidebar === 'Entradas' && <Inbox className="text-[#499D81] w-6 h-6" />}
                  {activeSidebar === 'Salidas' && <Send className="text-[#499D81] w-6 h-6" />}
                  {activeSidebar === 'Reportes' && <ClipboardList className="text-[#499D81] w-6 h-6" />}
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-slate-800 tracking-tight">
                    {activeSidebar === 'Entradas' ? 'Registro de Entradas' : 
                     activeSidebar === 'Salidas' ? 'Registro de Salidas' : 
                     'Gestión de Reportes'}
                  </h2>
                  <p className="text-slate-400 text-sm font-medium">
                    {activeSidebar === 'Entradas' ? 'Recepción e ingreso de productos' : 
                     activeSidebar === 'Salidas' ? 'Despacho y envío de productos' : 
                     'Monitoreo y exportación de actividades logísticas'}
                  </p>
                </div>
              </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {(activeSidebar === 'Entradas' ? mockData['Entrada'] : 
                      activeSidebar === 'Salidas' ? mockData['Salida'] : 
