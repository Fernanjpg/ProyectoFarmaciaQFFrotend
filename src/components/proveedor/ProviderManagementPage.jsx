...
export default function ProviderManagementPage() {
  ...
  const fetchProviders = async () => {
    try {
      const res = await fetch('http://localhost:8081/api/proveedores/Listar');
      if (res.ok) {
  ...
  const handleDeleteProvider = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:8081/api/proveedores/Eliminar/${id}`, {
        method: 'DELETE'
      });
      if (res.ok) {
        await fetchProviders();
  ...
  return (
    <Layout>
      <div className="p-8 lg:p-12 space-y-10">
        <ProviderTable
          providers={providers}
          onAddProviderClick={() => setIsModalOpen(true)}
