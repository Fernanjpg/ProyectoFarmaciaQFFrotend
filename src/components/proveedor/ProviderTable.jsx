import { Search, Plus, Trash2, Edit3, Truck } from 'lucide-react';

export default function ProviderTable({ providers, onAddProviderClick, onDeleteProvider }) {
  return (
      ...
      <div className="p-8 lg:p-10 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/30">
        <div className="flex items-center gap-5">
          ...
          <div>
            ...
            <p className="text-slate-400 text-sm font-medium mt-0.5">Socios comerciales y distribuidores farmacéuticos</p>
          </div>
        </div>
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="bg-slate-50/50">
              <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Nombre Comercial</th>
              <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Contacto / Teléfono</th>
              ...
              <tr key={p.id} className="group hover:bg-slate-50/50 transition-all duration-200">
                <td className="px-10 py-6">
                  <div className="flex items-center gap-4">
                    ...
                    <p className="text-sm font-black text-slate-700 group-hover:text-[#499D81] transition-colors">{p.nombre}</p>
                  </div>
                </td>
                <td className="px-10 py-6 text-sm text-slate-500 font-bold">{p.contacto || 'N/A'}</td>
                <td className="px-10 py-6">
